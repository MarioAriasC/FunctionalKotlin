...
import com.packtpub.functionalkotlin.chapter05.inTime
...
class ObservableUserService(private val userClient: UserClient,
							private val factClient: FactClient,
							private val userRepository: UserRepository,
							private val factRepository: FactRepository) : UserService {
	override fun getFact(id: UserId): Fact {

		return Flowable.fromCallable { Optional.ofNullable(userRepository.getUserById(id)) }
				.parallel(8)
				.runOn(Schedulers.io())
				.map { user ->
					...
					if (user.isPresent) {
						...
						userClient.getUser(id) to true
					}
				}
				.doOnNext { (user, toInsert) ->
					println(Thread.currentThread().name)
					if (toInsert) {
						userRepository.insertUser(user)
					}
				...
				.map { (user, inserted) ->
					println(Thread.currentThread().name)
					val (fact, fromDB) = if (inserted) {
						factClient.getFact(user) to false
					} else {
						factRepository.getFactByUserId(id) to true
					}
					Triple(fact, user, fromDB)
				}
				.map { (fact, user, fromDB) ->
					println(Thread.currentThread().name)
					if (fromDB) {
						if (fact == null) {
							factClient.getFact(user) to true
						} else {
							fact to false
						}
					} else {
		...
		/*return Single.fromCallable { Optional.ofNullable(userRepository.getUserById(id)) }
				.flatMap { user ->
					Single.fromCallable {
						println("flatMap observerOn")
						println(Thread.currentThread().name)
						if (!user.isPresent) {
							userClient.getUser(id) to true
						} else {
				...
				.flatMap { (fact, user, fromDB) ->
					Single.fromCallable {
						println("flatMap suscriberOn")
						println(Thread.currentThread().name)
						if (fromDB) {
							if (fact == null) {
								factClient.getFact(user) to true
...
fun main(args: Array<String>) = awaitEnter {
	...
	val userService = ObservableUserService(userClient,
			factClient,
			userRepository,
			factRepository)
